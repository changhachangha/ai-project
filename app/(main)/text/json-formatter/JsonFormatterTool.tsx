    ...
    const runJson = (minify: boolean) => {
        ...
        try {
            ...
            const result = processJson(
                ...
                { indentation: 2, minify, sortKeys: false }
            );
            if (!result.isValid) {
                throw new Error(result.errorMessage);
            }
            setJsonOutput(result.formattedJson);
            setError('');
            toast.success(minify ? 'JSON이 성공적으로 압축되었습니다!' : 'JSON이 성공적으로 포매팅되었습니다!');
        } catch (e: unknown) {
            const errorMessage = e instanceof Error ? e.message : String(e);
            setError(errorMessage);
            setJsonOutput('');
            toast.error(`JSON ${minify ? '압축' : '포매팅'} 실패: ${errorMessage}`);
        } finally {
                    ...
                    <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                        <div className='space-y-4'>
                            ...
                            <div className='flex gap-2'>
                                <Button onClick={() => runJson(false)} disabled={isProcessing} className='flex-1'>
